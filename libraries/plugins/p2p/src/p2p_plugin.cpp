...
#include "news/plugins/p2p/p2p_default_seeds.hpp"
        ...
        namespace p2p {
            namespace detail {
                ...
                public:
                    ...
                    p2p_plugin_impl(plugins::chain_plugin::chain_plugin *c)
                    ...
                    virtual news::base::chain_id_type get_chain_id() const override;

                    // node_delegate interface
                    virtual bool has_item(const graphene::net::item_id &) override;

                    virtual bool
                    handle_block(const graphene::net::block_message &, bool, std::vector<fc::uint160_t> &) override;
                    ...
                    virtual std::vector<graphene::net::item_hash_t>
                    get_block_ids(const std::vector<graphene::net::item_hash_t> &, uint32_t &, uint32_t) override;

                    virtual graphene::net::message get_item(const graphene::net::item_id &) override;

                    virtual std::vector<graphene::net::item_hash_t>
                    get_blockchain_synopsis(const graphene::net::item_hash_t &, uint32_t) override;
                    ...
                    plugins::chain_plugin::chain_plugin*chain;
...
////////////////////////////// Begin node_delegate Implementation //////////////////////////////
                ...
                { try {
                        if( running.load() )
                        {
                            ...
                            chain->get_database().with_read_lock( [&]()
                                                       {
                                                           head_block_num = chain->get_database().head_block_num();
                                                       });
                            if (sync_mode)
                            ...
                            try {
                               ...
                               // return result;
                                ...
                                FC_THROW_EXCEPTION(news::chain::unlinkable_block_exception, "Error when pushing block:\n${e}", ("e", e.to_detail_string()));
                        ...
                        return chain->get_database().with_read_lock( [&]()
                                                          {
                                                              ...
                                                              if( chain->get_database().head_block_num() == 0 )
                                                                  return result;
                                                              ...
                                                              for( uint32_t num = block_header::num_from_id(last_known_block_id);
                                                                   num <= chain->get_database().head_block_num() && result.size() < limit;
                                                                   ++num )
                                                              {
                                                                  if( num > 0 )
                                                              ...
                                                              if( !result.empty() && block_header::num_from_id(result.back()) < chain->get_database().head_block_num() )
                                                                  remaining_item_count = chain->get_database().head_block_num() - block_header::num_from_id(result.back());
                ...
                { try {
                        ...
                        return chain->get_database().with_read_lock( [&]()
                                                          {
                                                              return trx_message( chain->get_database().get_recent_transaction( id.item_hash ) );
                                                          });
                        ...
                        chain->get_database().with_read_lock( [&]()
                                                   {
                                                       ...
                                                       uint32_t low_block_num = chain->get_database().last_non_undoable_block_num();
                                                       ...
                                                       else
                                                       {
                                                           ...
                                                           high_block_num = chain->get_database().head_block_num();
                                                           non_fork_high_block_num = high_block_num;
                                                           if (high_block_num == 0)
                ...
                { try {
                        return chain->get_database().with_read_lock( [&]()
                                                          {
                                                              return chain->get_database().head_block_id();
                                                          });
                        ...
                        return chain->get_database().with_read_lock( [&]()
                                                          {
                                                              ...
                                                              block_id_type block_id_in_preferred_chain = chain->get_database().get_block_id_for_num(block_num);
...
////////////////////////////// End node_delegate Implementation //////////////////////////////
            ...
            void p2p_plugin::set_program_options(bpo::options_description &cli, bpo::options_description &cfg) {
             ...
             //   for (auto &s : default_seeds) {
              ...
              //  }
                ...
                cfg.add_options()
                        ("p2p-endpoint", bpo::value<string>()->implicit_value("127.0.0.1:9876"),
                         "The local IP address and port to listen for incoming connections.")
                        ("p2p-max-connections", bpo::value<uint32_t>(),
                         "Maxmimum number of incoming connections on P2P endpoint.")
                        ("seed-node", bpo::value<vector<string>>()->composing(),
                         "The IP address and port of a remote peer to sync with. Deprecated in favor of p2p-seed-node.")
                        ("p2p-seed-node",
                         bpo::value<vector<string>>()->composing()->default_value(default_seeds, seed_ss.str()),
                ...
                cli.add_options()
                        ...
                        ("p2p-force-validate", bpo::bool_switch()->default_value(false),
                         "Force validation of all transactions.");
            }

            void p2p_plugin::plugin_initialize(const boost::program_options::variables_map &options) {
                auto plg=  news::app::application::getInstance().find_plugin<news::plugins::chain_plugin::chain_plugin>();
                my = std::make_unique<detail::p2p_plugin_impl>(plg);
                ...
                if (options.count("p2p-parameters")) {
                    fc::variant var = fc::json::from_string(options.at("p2p-parameters").as<string>(),
                                                            fc::json::strict_parser);
            ...
            void p2p_plugin::plugin_startup() {
                my->p2p_thread.async([this] {
                    ...
                    auto path= news::app::application::getInstance().get_data_path() / "p2p";
                    my->node->load_configuration(path);
                    ...
                    if (my->max_connections) {
                        if (my->config.find("maximum_number_of_connections") != my->config.end())
                            ilog("Overriding advanded_node_parameters[ \"maximum_number_of_connections\" ] with ${cons}",
                                 ("cons", my->max_connections));
