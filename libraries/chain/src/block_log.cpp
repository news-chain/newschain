...
namespace news{
    namespace chain{
        ...
        uint64_t block_log::append(const signed_block& b) {
            try {
                ...
                FC_ASSERT(my->index_stream.tellp() == sizeof(uint64_t) * (b.block_num() - 1),
                          "Append to index file occuring at wrong position. ${position}",
        ...
        fc::optional<signed_block> block_log::read_block_by_num(uint32_t block_num)const {
            try {
        ...
        fc::optional<signed_block> block_log::read_head()const {
            my->check_block_read();
        ...
        const fc::optional<signed_block>& block_log::head()const {
            return my->head;
        }
