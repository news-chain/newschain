    ...
    namespace plugins{
        namespace json_rpc{
            namespace detail{
                ...
                typedef std::vector< std::string >   get_methods_return;
                ...
                {
                public:
                    json_rpc_logger(const std::string& _dir_name) : dir_name(_dir_name) {}
                ...
                void json_rpc_plugin_impl::add_api_method(const string &api_name, const string &method_name,
                ...
                api_method *json_rpc_plugin_impl::process_params(string method, const fc::variant_object &request,
...
//                                            ilog("func_args ${p}",("p", func_args));
            ...
            void json_rpc_plugin::set_program_options(options_description &, options_description &cfg) {
                cfg.add_options()("log-json-rpc", bpo::value<std::string>(), "json-rpc-log directory name.");
            }

            void json_rpc_plugin::plugin_initialize(const variables_map &options) {
                my->initialize();
                if(options.count("log-json-rpc")){
                    auto dir_name = options.at("log-json-rpc").as<std::string>();
                    ...
                    my->_logger.reset(new detail::json_rpc_logger(dir_name));
                }
            }

            void json_rpc_plugin::plugin_startup() {
                std::sort(my->_methods.begin(), my->_methods.end());
                for(auto &m : my->_methods){
                    ilog("json_rpc_plugin init method: ${e}", ("e", m));
                }
            }
            ...
            void json_rpc_plugin::add_api_method(const std::string &api_name, const std::string &method_name,
                                                 const api_method &api, const api_method_signature &sig) {
                my->add_api_method(api_name, method_name, api, sig);

            }

            std::string json_rpc_plugin::call(const std::string &body) {
                try {
                    fc::variant v = fc::json::from_string(body);
                    if(v.is_array()){
                        std::vector<fc::variant> msg = v.as< std::vector< fc::variant > >();
                        std::vector< detail::json_rpc_response > responses;

                        if(msg.size()){
                            responses.reserve(msg.size());
                            for(auto &m : msg){
                                responses.push_back(my->rpc(m));
                            }
                            return fc::json::to_string(responses);
                        }else{
                            detail::json_rpc_response response;
                            response.error = detail::json_rpc_error( JSON_RPC_SERVER_ERROR, "Array is invalid");
...
namespace fc{
    void to_variant(const news::plugins::json_rpc::detail::json_rpc_error &error, variant &var){
        try {
            var = fc::mutable_variant_object("code", error.code)("message", error.message)("data", error.data);
        }FC_CAPTURE_AND_RETHROW()
    }


    void to_variant(const news::plugins::json_rpc::detail::json_rpc_response &response, variant &var){
        try {
            var = fc::mutable_variant_object("jsonrpc", response.jsonrpc)("result", response.result)("error", response.error)("id", response.id);
        }FC_CAPTURE_AND_RETHROW()
    }
}
...
FC_REFLECT( news::plugins::json_rpc::detail::json_rpc_error, (code)(message)(data) )
FC_REFLECT( news::plugins::json_rpc::detail::json_rpc_response, (jsonrpc)(result)(error)(id) )

FC_REFLECT( news::plugins::json_rpc::detail::get_signature_args, (method) )
