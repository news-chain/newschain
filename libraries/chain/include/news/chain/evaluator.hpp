...
namespace news{
    namespace chain{
        ...
        template< typename OperationType=news::base::operation >
        class evaluator
        {
        public:
            virtual ~evaluator() {}
        ...
        template< typename EvaluatorType, typename OperationType=news::base::operation >
...
#define NEWS_DEFINE_EVALUATOR( X ) \
class X ## _evaluator : public news::chain::evaluator_impl< X ## _evaluator > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( news::chain::database& db )                                       \
         : news::chain::evaluator_impl< X ## _evaluator >( db )          \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
};

#define NEWS_DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )               \
class X ## _evaluator : public news::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( news::chain::database& db, PLUGIN* plugin )       \
         : news::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ), \
           _plugin( plugin )                                                \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
